...
const sendSignInCode = async (email: string, code: string) => {
  ...
  await sendMail({
    html: `<p>Чтобы войти в Purr, откройте ссылку:</p><p><a href="${link}">Войти</a></p><p>Или введите код: <b>${code}</b></p><p>Ссылка и код действуют 10 минут.</p>`,
    subject: `Код для входа в Purr: ${code}`,
    text: `Чтобы войти в Purr, откройте ссылку: ${link}\n\nИли введите код: ${code}\n\nСсылка и код действуют 10 минут.`,
    to: email,
  });
};
